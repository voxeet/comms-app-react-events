...
export const communications = {
  streaming: {
    startRts: async (token: string, conferenceId: string) => {
      const url = encodeURI(
        `${commsBaseUrl}/v3/conferences/mix/${conferenceId}/rts/start`
      );
    ...
    stopRts: async (token: string, conferenceId: string) => {
      const url = encodeURI(
        `${commsBaseUrl}/v3/conferences/mix/${conferenceId}/rts/stop`
      );
...
router.post("/event/start", async ({ body }, res) => {
  ...
  try {
    ...
    const { viewerUrl, status, ...rest } =
      await communications.streaming.startRts(token, conferenceId);
    if (viewerUrl) {
